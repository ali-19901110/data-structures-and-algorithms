...
let newarr =arr.map((val)=>{
  let nstr =  val['name'].split("").reverse().join("");
  result.push(nstr);
})
return result;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let narr = input.map(element => element.filter (value => typeof(value) === 'number' && !(value % 5))); 
  let res = narr.map(val => val.map(elem => Math.pow(2,elem))); 
  return res;
};
...
let findShortest = (data) => {
  // Solution code here...
  let sor= data.sort((elemnt1,elemnt2) => elemnt1.height - elemnt2.height );
  return sor[0].name;
};
